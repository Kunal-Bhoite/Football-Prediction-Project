...
model = SVC(C=0.1, kernel='rbf', gamma='scale', random_state=42)
...
if prediction[0] == 0:
    ...
    draw.append(home_team)
    draw.append(away_team)

# Ensure winner list has at least 6 elements before accessing index 5
if len(winner) == 5:
    winner.append(home_team)

# Print winner at index 5
if len(winner) >= 6:
    print(winner[5])
else:
    print("No winner at index 5")
winner[5]
...
prediction = model.predict(new_entry_preprocessed)
if prediction[0] == 0:
    winner_sf.append(newentry["home_team"][0])
    loser_sf.append(newentry["away_team"][0])
elif prediction[0] == 1:
    winner_sf.append(newentry["away_team"][0])
    loser_sf.append(newentry["away_team"][0])
else:
    draw.append(newentry["home_team"][0])
    draw.append(newentry["away_team"][0])
...
winner_sf = ['Philippines','Haiti']

loser_sf=['Zambia','Morocco']
